...
WasapiFormat::WasapiFormat(const WAVEFORMATEX& format)
  ...
  if (sample_rate_ == 0 || channels_ == 0 || block_align_ == 0 ||
      container_bits_ == 0 || block_align_ % channels_ != 0) {
  ...
  if (is_extensible(format)) {
    ...
    if (IsEqualGUID(extended.SubFormat, KSDATAFORMAT_SUBTYPE_IEEE_FLOAT)) {
      ...
      throw std::runtime_error("unsupported WASAPI endpoint sample subtype");
    }
  ...
  if (encoding_ == Encoding::ieee_float && container_bits_ != 32 &&
      container_bits_ != 64) {
    throw std::runtime_error("unsupported WASAPI floating-point sample width");
  }
  if (encoding_ == Encoding::signed_pcm &&
      (container_bits_ < 16 || container_bits_ > 32 ||
       valid_bits_ > container_bits_)) {
...
void WasapiFormat::decode_mono(const BYTE* input, const std::size_t frame_count,
  ...
  if (output.size() < frame_count) {
    throw std::invalid_argument("mono decode output is too small");
  }
